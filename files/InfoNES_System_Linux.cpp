...
static uint32_t convert_RGB565_to_RGB8888(WORD color)
{
        uint8_t r = ((color & 0xF800) >> (11 - 3));
        uint8_t g = ((color & 0x07E0) >> (5 - 2));
        uint8_t b = ((color & 0x001F) << 3);

        // do bit-replication to allow full-scale-values
        r |= (r >> 5);
        g |= (g >> 6);
        b |= (b >> 5);
...
*/
	...
	// get pointer to framebuffer and draw color to this pixel
	uint32_t *pen32;
	pen32 = (uint32_t *)(fb_mem + y*line_width + x*px_width);
	*pen32 = convert_RGB565_to_RGB8888(color);
...
/* Palette data (Nestopia RGB565) */
WORD NesPalette[64] =
{
        0x52AA, 0x00EE, 0x0892, 0x3011, 0x400C, 0x5806, 0x5020, 0x38C0,
        0x2140, 0x0180, 0x0180, 0x0143, 0x0107, 0x0000, 0x0000, 0x0000,
        0x94D2, 0x0A72, 0x29F7, 0x4998, 0x6954, 0x78CD, 0x7087, 0x58E0,
        0x31A0, 0x0240, 0x0220, 0x0203, 0x01EA, 0x0000, 0x0000, 0x0000,
        0xEF5D, 0x4CDD, 0x7BFD, 0xA3DD, 0xDB7C, 0xF375, 0xF3AC, 0xEC87,
        0xC540, 0x8DE0, 0x6641, 0x3628, 0x064F, 0x0000, 0x0000, 0x0000,
        0xEF5D, 0xA65D, 0xC63D, 0xD5FD, 0xF5FD, 0xFDDB, 0xFE36, 0xF6B3,
        0xE6F0, 0xCF2F, 0xB76F, 0x9F73, 0x9F7B, 0x0000, 0x0000, 0x0000
};
